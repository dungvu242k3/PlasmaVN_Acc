...
if (!supabaseUrl || !supabaseAnonKey) {
    fs.writeFileSync('mockDataResult.txt', 'Missing Supabase credentials');
    process.exit(1);
}
...
async function injectMockData() {
    try {
        ...
        if (error) {
            fs.writeFileSync('mockDataResult.txt', JSON.stringify(error, null, 2));
        } else {
            fs.writeFileSync('mockDataResult.txt', 'SUCCESS!');
        }
    } catch (e) {
        fs.writeFileSync('mockDataResult.txt', e.stack || e.message);
    }
}
